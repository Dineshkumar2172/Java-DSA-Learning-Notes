...
public class Dijkstra {
    ...
    public void executeDijkstra(Integer srcVertex, List<List<Integer[]>> adjList) {
        int n = adjList.size();
        ...
        while (!pq.isEmpty()) {
            ...
            for (Integer[] neighbor: adjList.get(currentVertex)) {
                if (currentWeight + neighbor[1] < shortestPath[neighbor[0]]) {
                    shortestPath[neighbor[0]] = currentWeight + neighbor[1];
                    pq.offer(new Integer[]{neighbor[0], currentWeight + neighbor[1]});
                }
            }
        }

        System.out.println("shortest distande from vertex " + srcVertex + ": ");
        for (int i = 0; i < n; i++) {
